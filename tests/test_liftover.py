...
def test_lift_over_variants():
    ...
    for i, lifted_variant in enumerate(result_hg38):
        assert lifted_variant != "", f"Variant {i} ({hg19_variants[i]}) failed to lift over"
        assert len(lifted_variant.split("-")) == 4, f"Malformed lifted variant: {lifted_variant}"
    
    # Test specific expected results (gold standard)
    for i, (original, lifted, expected) in enumerate(zip(hg19_variants, result_hg38, expected_hg38_variants)):
        assert lifted == expected, f"Row {i}: {original} -> Expected {expected}, got {lifted}"
...
def test_lift_over_variants_reverse():
    ...
    for i, lifted_variant in enumerate(result_hg19):
        assert lifted_variant != "", f"Variant {i} ({hg38_variants[i]}) failed to lift over"
        assert len(lifted_variant.split("-")) == 4, f"Malformed lifted variant: {lifted_variant}"
    ...
    for original, lifted in zip(hg38_variants, result_hg19):
        ...
        assert orig_pos != lifted_pos, f"Position unchanged in liftover: {original} -> {lifted}"
...
def test_lift_over_variants_df():
    ...
    # Test column names
    expected_columns = ["chr", "pos", "ref", "alt", "chr_lifted", "pos_lifted", "ref_lifted", "alt_lifted"]
    ...
    assert result_df["pos_lifted"].dtype in ["int64", "Int64", "object"]  # Could be various types
    ...
    assert position_changes >= 3, f"Expected at least 3 position changes, got {position_changes}"
    ...
    assert chr_prefixes >= 3, f"Expected chr prefixes in lifted coordinates, got {chr_prefixes}"
...
def test_lift_over_different_genomes():
    ...
    for from_genome, dest_genome in genome_combinations:
        ...
        assert len(result[0].split("-")) == 4, f"Malformed result for {from_genome} -> {dest_genome}: {result[0]}"
...
def test_lift_over_large_batch():
    ...
    failure_rate = (len(large_variant_set) - len(successful_lifts)) / len(large_variant_set)
    
    # Allow up to 20% failure rate (some positions might not exist or be problematic)
    assert failure_rate <= 0.2, f"Too many failures: {len(successful_lifts)}/{len(large_variant_set)} succeeded"
    
    print(f"Large batch test: {len(successful_lifts)}/{len(large_variant_set)} variants lifted successfully")
